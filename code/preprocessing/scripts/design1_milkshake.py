...
def fill_dict(sub):
    ...
    curr_funcs = glob.glob(os.path.join(data_dir, 'func', '*milkshake*brain.nii.gz'))
    ...
    for func in curr_funcs:
        ...
        anat = os.path.join(data_dir, 'anat', 'highres.nii.gz')
    ...
    # -- CONFOUNDS
        confound=os.path.join(data_dir, 'func','motion_assessment','%s_%s_task-%s_bold_space-MNI152NLin2009cAsym_preproc_brain_confound.txt'%(sub,arglist["SESS"], task))
    ...
    # -- EVS -- here we loop through the given EVs and add the corresponding file to the dictionary
        ...
        for ev_id in arglist['EV']:
            ...
            """ h2O_receipt HF_HS_receipt HF_LS_receipt LS_HS_receipt LF_LS_receipt milkshake_pic h2O_pic rinse """
